...
@Component
public class UserSchedulerUsingKafka {
    ...
    public void fetchUserAndSendSAMail(){
        ...
        for(User user : userForSA){
            ...
            if(mostFrequentSentiment != null){
                ...
                SentimentData sentimentData = SentimentData.builder().email(user.getEmail()).sentiment("Hii "+name+", Your sentiment for last 7 days is " + mostFrequentSentiment).build();
                try{
                    kafkaTemplate.send("weekly-sentiments", sentimentData.getEmail(), sentimentData);
                } catch (Exception e) {
                    emailService.sendEmail(user.getEmail(), "Sentiment for last 7 days", "Hi "+name+", Your sentiment for last 7 days is "+mostFrequentSentiment.toString());
                }
